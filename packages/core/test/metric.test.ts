...
class InjectableMetricsController {
  ...
  gaugeDecNoData() { this.gauge.dec() }
  gaugeInc() { this.gauge.inc(...withValues('gauge')) }
  gaugeIncNoData() { this.gauge.inc() }
...
describe('src/module', function () {
  ...
  describe('Gauge', () => {
    ...
    it(`Gauge.startTimer(${JSON.stringify(withValues('gauge'))}) should be called with proper values`, async () => {
      const controller = harness.app.get<InjectableMetricsController>(InjectableMetricsController)
      const endTimer = await controller.gaugeStartTimer();

      expect(controller.gauge.startTimer).to.have.been.called;
      expect(controller.gauge.startTimer).to.have.been.calledWith(...withValues2('gauge'));
      expect(endTimer).to.have.been.called;
      expect(endTimer).to.have.been.calledWith(...withValues3('gauge'));
    });
  ...
  describe('Histogram', () => {
    ...
    it(`Histogram.observe(${JSON.stringify(withValues('histogram'))}) should be called with proper values`, async () => {
      const controller = harness.app.get<InjectableMetricsController>(InjectableMetricsController)
    ...
    it(`Histogram.startTimer(${JSON.stringify(withValues('histogram'))}) should be called with proper values`, async () => {
      const controller = harness.app.get<InjectableMetricsController>(InjectableMetricsController)
      ...
      expect(endTimer).to.have.been.called;
      expect(endTimer).to.have.been.calledWith3(...withValues('histogram'));
    });
  ...
  describe('Summary', () => {
    ...
    it(`Summary.startTimer(${JSON.stringify(withValues('summary'))}) should be called with proper values`, async () => {
      ...
      expect(endTimer).to.have.been.called;
      expect(endTimer).to.have.been.calledWith3(...withValues('summary'));
    });
